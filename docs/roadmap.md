...
| 11d | M6 Double DQN training-budget check | `scripts/eval_double_dqn_budget.py` | done — 2,000 vs 5,000 episodes across 10 matched seeds; selected and exact-endpoint estimands |
| 11e | M6 recurrent Double DQN | `src/rl/recurrent_dqn.py`, `scripts/eval_recurrent_double_dqn.py` | done — near-parameter-matched feed-forward vs GRU comparison; 10 seeds, selected and exact endpoints |
| 11f | M6 recurrent-memory isolation | `src/rl/episode_replay_dqn.py`, `scripts/eval_memory_ablation.py` | done — complete-episode replay, 150 transitions/loss, and 62,000 updates matched across MLP and GRU; 10 seeds |
| 11g | M7 multi-skill prerequisite simulator | `src/rl/multiskill_environment.py`, `scripts/validate_multiskill_env.py` | done — separate mastery per skill, validated DAG, prerequisite gating/transfer, oracle/estimated/no-state observations |
...
| 2026-09-04 | `9e7901d` + budget worktree | Double DQN 2,000 vs 5,000 episodes, 10 matched seeds, 500 common held-out episodes; selected + exact endpoints | `configs/double_dqn_train_5000.yaml`, `configs/double_dqn_budget_evaluation.yaml` | Validation-selected mean improves 0.5676→0.5756, +0.0080 [0.0018, 0.0157]; exact endpoint worsens 0.5556→0.5414, -0.0142 [-0.0230, -0.0026]. SD reductions are inconclusive; early stopping is essential. |
| 2026-09-05 | `adccc4b` + recurrent worktree | feed-forward vs GRU Double DQN, 5,000 episodes, 10 matched seeds, 500 common held-out episodes; selected + exact endpoints | `configs/recurrent_double_dqn_train.yaml`, `configs/recurrent_double_dqn_evaluation.yaml` | Selected GRU 0.8374 vs feed-forward 0.5756, +0.2618 [0.2555, 0.2691]; exact endpoint 0.8212 vs 0.5414, +0.2798 [0.2684, 0.2907]; GRU wins 10/10 in both. Lower SD is inconclusive; recurrent replay is a confound. |
| 2026-09-05 | `adccc4b` + memory-ablation worktree | feed-forward vs GRU Double DQN with matched complete-episode replay, 10 seeds, 500 common held-out episodes | `configs/episode_replay_double_dqn_train.yaml`, `configs/memory_ablation_evaluation.yaml` | Selected GRU 0.8374 vs matched MLP 0.5742, +0.2632 [0.2577, 0.2695]; exact endpoint 0.8212 vs 0.5525, +0.2687 [0.2563, 0.2805]; GRU wins 10/10 in both. Replay/training exposure does not explain the gain. |
| 2026-09-05 | `adccc4b` + multi-skill worktree | four-skill diamond prerequisite simulator, 500 paired calibration episodes | `configs/multiskill_simulator.yaml` | Prerequisite-aware heuristic final mean mastery 0.3455 vs current-only 0.3263, +0.0192 [0.0188, 0.0196]; readiness 0.6040 vs 0.5466. Rule over-reviews foundations, leaving a learned planning problem. |
