"""Train feed-forward Double DQN with complete-episode replay."""
...
from src.rl.dqn import linear_epsilon
...
def episode_replay_agent_config(cfg: dict) -> EpisodeReplayDQNConfig:
    dqn = cfg["dqn"]
    replay = cfg["episode_replay_dqn"]
    return EpisodeReplayDQNConfig(
        hidden_dims=tuple(int(value) for value in dqn["hidden_dims"]),
        gamma=float(cfg["rl"]["gamma"]),
        learning_rate=float(dqn["learning_rate"]),
        episode_batch_size=int(replay["episode_batch_size"]),
        replay_capacity_episodes=int(replay["replay_capacity_episodes"]),
        min_replay_episodes=int(replay["min_replay_episodes"]),
...
def main(
    ...
    agent = EpisodeReplayDQNAgent(
        ...
        episode_replay_agent_config(cfg),
        device,
        seed=seed,
    )
    ...
    stem = "episode_replay_dqn_oracle"
    ...
    print(
        "Episode-replay feed-forward Double DQN | "
        f"device={device} | seed={seed} | episodes={train_episodes} | "
        f"parameters={agent.parameter_count}"
    )
    print(
        "CONTROL: complete 50-step episodes, 150 transitions/loss, and "
        "episode-boundary updates match the GRU training scheme."
    )
    print(
    ...
    common_extra = {
        ...
        "replay_scheme": "complete_episode_flattened",
    }
    agent.save(
        best_path,
        extra={
    ...
    best_agent, best_extra = EpisodeReplayDQNAgent.load(best_path, device)
    ...
    payload = {
        "disclaimer": (
            "Simulated episode-replay feed-forward Double DQN training only. "
            "Oracle-mastery outcomes are not real-student evidence."
        ),
        "device": str(device),
        "algorithm": "episode_replay_double_dqn",
        "architecture": "feedforward",
        "replay_scheme": "complete_episode_flattened",
        "seed": seed,
        "observation_condition": tracking.condition,
        ...
        "episode_replay_config": cfg["episode_replay_dqn"],
        ...
        "provenance": {
            **git_provenance(),
            "sha256": {
                ...
                "episode_replay_dqn": sha256(Path("src/rl/episode_replay_dqn.py")),
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--config", default="configs/episode_replay_double_dqn_train.yaml"
    )
